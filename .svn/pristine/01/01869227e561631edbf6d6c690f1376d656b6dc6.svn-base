...
function startSlider() {
    ...
    if (this_index + 1 < this_count) {
        var new_index = this_index + 1;
    } else {
        var new_index = 0;
    }
...
function printArticle() {
    ...
    var link_add = jQuery(".main-article-content h1.article-title").attr('data-content');
    
    if (jQuery(".main-article-content .slider-content img").size() > 0) {
        var htmlphoto = "<img src='" + jQuery(".main-article-content .slider-content img").attr("src") + "' alt='' style='max-width: 600px;'/>";
    } else {
        var htmlphoto = "";
    }
    if (jQuery(".logo-image img").size() > 0) {
        var htmllogo = '<div class="logo-image">' + jQuery(".logo-image").html() + '</div>';
        var logotext = false;
    } else {
        var htmllogo = '<div class="logo-text">' + jQuery(".logo-text").html() + '</div>';
        var logotext = true;
    }
    ...
    top.consoleRef.document.writeln(
            ...
            + htmllogo
            + '<h2>' + htmltitle + '</h2>'
            + '<div class="smallinfo"><strong>' + htmlauthor + '</strong><span>' + htmldate + '</span><a href="' + jQuery(location).attr('href') + '">' + link_add + '</a></div><br />'
